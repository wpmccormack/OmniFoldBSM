...
aH = np.load("/data0/users/pkomiske/OmniFoldSearch/Herwig_ZJet.npz")

sjetsaH = aH['sim_jets'][:600000]
gjetsaH = aH['gen_jets'][:600000]
gZsaH = aH['gen_Zs'][:600000]

del aH

bH = np.load("/data0/users/pkomiske/OmniFoldSearch/Herwig_ZJet_Obs.npz")
smultsbH = bH['sim_mults'][:600000]
gmultsbH = bH['gen_mults'][:600000]

snsubsbH = bH['sim_nsubs'][:600000]
gnsubsbH = bH['gen_nsubs'][:600000]

ssdmsbH = bH['sim_sdms'][:600000]
gsdmsbH = bH['gen_sdms'][:600000]

szgsbH = bH['sim_zgs'][:600000]
gzgsbH = bH['gen_zgs'][:600000]

sn95sbH = bH['sim_n95s'][:600000]
gn95sbH = bH['gen_n95s'][:600000]
...
for m in masses:
    ...
    snsubsd.append(d['sim_nsubs'][:])
    gnsubsd.append(d['gen_nsubs'][:])

    ssdmsd.append(d['sim_sdms'][:])
    gsdmsd.append(d['gen_sdms'][:])

    szgsd.append(d['sim_zgs'][:])
    gzgsd.append(d['gen_zgs'][:])

    sn95sd.append(d['sim_n95s'][:])
    gn95sd.append(d['gen_n95s'][:])
...
invmaTH = []
print(len(gjetsaH))
for i in range(len(gjetsaH)):
    ej = np.sqrt(gjetsaH[i][3]*gjetsaH[i][3] + gjetsaH[i][0]*np.cosh(gjetsaH[i][1])*gjetsaH[i][0]*np.cosh(gjetsaH[i][1]))
    ez = np.sqrt(90.*90. + gZsaH[i][0]*np.cosh(gZsaH[i][1])*gZsaH[i][0]*np.cosh(gZsaH[i][1]))
    eb = ej+ez
    pxb = gjetsaH[i][0]*np.cos(gjetsaH[i][2]) + gZsaH[i][0]*np.cos(gZsaH[i][2])
    pyb = gjetsaH[i][0]*np.sin(gjetsaH[i][2]) + gZsaH[i][0]*np.sin(gZsaH[i][2])
    pzb = gjetsaH[i][0]*np.sinh(gjetsaH[i][1]) + gZsaH[i][0]*np.sinh(gZsaH[i][1])
    MB = np.sqrt(eb*eb - pxb*pxb - pyb*pyb - pzb*pzb)
    invmaTH.append([MB,gjetsaH[i][3],gmultsbH[i],gjetsaH[i][0],gZsaH[i][0],gnsubsbH[i][0],gnsubsbH[i][1],gnsubsbH[i][2],gsdmsbH[i][0],gzgsbH[i][0],gn95sbH[i]])
...
invmaH = []
print(len(sjetsaH))
for i in range(len(sjetsaH)):
    ej = np.sqrt(sjetsaH[i][3]*sjetsaH[i][3] + sjetsaH[i][0]*np.cosh(sjetsaH[i][1])*sjetsaH[i][0]*np.cosh(sjetsaH[i][1]))
    ez = np.sqrt(90.*90. + gZsaH[i][0]*np.cosh(gZsaH[i][1])*gZsaH[i][0]*np.cosh(gZsaH[i][1]))
    eb = ej+ez
    pxb = sjetsaH[i][0]*np.cos(sjetsaH[i][2]) + gZsaH[i][0]*np.cos(gZsaH[i][2])
    pyb = sjetsaH[i][0]*np.sin(sjetsaH[i][2]) + gZsaH[i][0]*np.sin(gZsaH[i][2])
    pzb = sjetsaH[i][0]*np.sinh(sjetsaH[i][1]) + gZsaH[i][0]*np.sinh(gZsaH[i][1])
    MB = np.sqrt(eb*eb - pxb*pxb - pyb*pyb - pzb*pzb)
    invmaH.append([MB,sjetsaH[i][3],smultsbH[i],sjetsaH[i][0],gZsaH[i][0],snsubsbH[i][0],snsubsbH[i][1],snsubsbH[i][2],ssdmsbH[i][0],szgsbH[i][0],sn95sbH[i]])
...
Mdata = np.concatenate([invmc[int(sys.argv[1])][:nsig],invmaH[nevs:nevs+nbkg]])
Msim = np.concatenate([invma[:nevs]])
Mgen = np.concatenate([invmaT[:nevs]])

MdataT = np.concatenate([invmcT[int(sys.argv[1])][:nsig],invmaTH[nevs:nevs+nbkg]])
...
np.savez('/data0/users/wmccorma/'+str(percname)+'PercSig_'+str(sys.argv[1])+'MeV_NOsynthsig_multifold_herwig.npz', **{'Y_det': Y_det, 'Y_gen': Y_gen, 'Y_detT': Y_detT, 'X_det_glob': MtmpX_det, 'X_gen_glob': MtmpX_gen, 'X_detT_glob': MtmpX_detT})
