...
a = np.load("/data0/users/pkomiske/OmniFoldSearch/Herwig_ZJet.pickle", allow_pickle=True)

HWbkg_sim = a['sim_particles'][:600000]
HWbkg_gen = a['gen_particles'][:600000]

del a

a = np.load("/data0/users/pkomiske/OmniFoldSearch/Herwig_ZJet.npz")

HWsjetsa = a['sim_jets'][:600000]
HWgjetsa = a['gen_jets'][:600000]
HWgZsa = a['gen_Zs'][:600000]

del a

b = np.load("/data0/users/pkomiske/OmniFoldSearch/Herwig_ZJet_Obs.npz")
HWsmultsb = b['sim_mults'][:600000]
HWgmultsb = b['gen_mults'][:600000]
...
i=0
numdel = 0
while(i<len(HWbkg_sim)):
    if( (abs(np.sum(HWbkg_gen[i])) < 0.001) or (abs(np.sum(HWbkg_sim[i])) < 0.001) or (HWsmultsb[i] == 0) or (HWgmultsb[i] == 0)):
        HWbkg_sim = np.delete(HWbkg_sim,i,0)
        HWbkg_gen = np.delete(HWbkg_gen,i,0)
        HWsjetsa = np.delete(HWsjetsa,i,0)
        HWgjetsa = np.delete(HWgjetsa,i,0)
        HWgZsa = np.delete(HWgZsa,i,0)
        HWsmultsb = np.delete(HWsmultsb,i,0)
        HWgmultsb = np.delete(HWgmultsb,i,0)
        numdel+=1
    else:
        i+=1
print(numdel)
...
HWinvmaT = []
print(len(HWgjetsa))
for i in range(len(HWgjetsa)):
    ej = np.sqrt(HWgjetsa[i][3]*HWgjetsa[i][3] + HWgjetsa[i][0]*np.cosh(HWgjetsa[i][1])*HWgjetsa[i][0]*np.cosh(HWgjetsa[i][1]))
    ez = np.sqrt(90.*90. + HWgZsa[i][0]*np.cosh(HWgZsa[i][1])*HWgZsa[i][0]*np.cosh(HWgZsa[i][1]))
    eb = ej+ez
    pxb = HWgjetsa[i][0]*np.cos(HWgjetsa[i][2]) + HWgZsa[i][0]*np.cos(HWgZsa[i][2])
    pyb = HWgjetsa[i][0]*np.sin(HWgjetsa[i][2]) + HWgZsa[i][0]*np.sin(HWgZsa[i][2])
    pzb = HWgjetsa[i][0]*np.sinh(HWgjetsa[i][1]) + HWgZsa[i][0]*np.sinh(HWgZsa[i][1])
    MB = np.sqrt(eb*eb - pxb*pxb - pyb*pyb - pzb*pzb)
    HWinvmaT.append([MB,HWgjetsa[i][3],HWgmultsb[i]])
...
HWinvma = []
print(len(HWsjetsa))
for i in range(len(HWsjetsa)):
    ej = np.sqrt(HWsjetsa[i][3]*HWsjetsa[i][3] + HWsjetsa[i][0]*np.cosh(HWsjetsa[i][1])*HWsjetsa[i][0]*np.cosh(HWsjetsa[i][1]))
    ez = np.sqrt(90.*90. + HWgZsa[i][0]*np.cosh(HWgZsa[i][1])*HWgZsa[i][0]*np.cosh(HWgZsa[i][1]))
    eb = ej+ez
    pxb = HWsjetsa[i][0]*np.cos(HWsjetsa[i][2]) + HWgZsa[i][0]*np.cos(HWgZsa[i][2])
    pyb = HWsjetsa[i][0]*np.sin(HWsjetsa[i][2]) + HWgZsa[i][0]*np.sin(HWgZsa[i][2])
    pzb = HWsjetsa[i][0]*np.sinh(HWsjetsa[i][1]) + HWgZsa[i][0]*np.sinh(HWgZsa[i][1])
    MB = np.sqrt(eb*eb - pxb*pxb - pyb*pyb - pzb*pzb)
    HWinvma.append([MB,HWsjetsa[i][3],HWsmultsb[i]])
...
data = np.concatenate([sig_sim[int(sys.argv[1])][:nsig],HWbkg_sim[nevs:nevs+nbkg]])
...
dataT = np.concatenate([sig_gen[int(sys.argv[1])][:nsig],HWbkg_gen[nevs:nevs+nbkg]])
...
Mdata = np.concatenate([invmc[int(sys.argv[1])][:nsig],HWinvma[nevs:nevs+nbkg]])
...
MdataT = np.concatenate([invmcT[int(sys.argv[1])][:nsig],HWinvmaT[nevs:nevs+nbkg]])
...
#np.savez('/data0/users/wmccorma/'+str(percname)+'PercSig_'+str(sys.argv[1])+'MeV_synthsig.npz', **{'X_det_part': tmpX_det, 'Y_det': Y_det, 'X_gen_part': tmpX_gen, 'Y_gen': Y_gen, 'X_detT_part': tmpX_detT, 'Y_detT': Y_detT, 'X_det_glob': MtmpX_det, 'X_gen_glob': MtmpX_gen, 'X_detT_glob': MtmpX_detT})
np.savez('/data0/users/wmccorma/'+str(percname)+'PercSig_'+str(sys.argv[1])+'MeV_Herwig_DataWithBSM_synthsig.npz', **{'X_det_part': tmpX_det, 'Y_det': Y_det, 'X_gen_part': tmpX_gen, 'Y_gen': Y_gen, 'X_detT_part': tmpX_detT, 'Y_detT': Y_detT, 'X_det_glob': MtmpX_det, 'X_gen_glob': MtmpX_gen, 'X_detT_glob': MtmpX_detT})
