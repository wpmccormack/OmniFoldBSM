...
bkg_sim = a['sim_particles'][:550000]
bkg_gen = a['gen_particles'][:550000]
...
themax = max(sim_data_max_length, simT_dataT_max_length)

print(sim_data_max_length)
print(themax)

gen, sim = pad_events(gen, max_length=themax), pad_events(sim, max_length=themax)
data = pad_events(data, max_length=themax)

simT = pad_events(simT, max_length=themax)
dataT = pad_events(dataT, max_length=themax)
...
np.savez('/data0/users/wmccorma/'+str(percname)+'PercSig_mini_'+str(sys.argv[1])+'MeV.npz', **{'X_det': X_det, 'Y_det': Y_det, 'X_gen': X_gen, 'Y_gen': Y_gen, 'X_detT': X_detT, 'Y_detT': Y_detT})
