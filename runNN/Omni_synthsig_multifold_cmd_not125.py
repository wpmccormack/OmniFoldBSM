...
fi = np.load('/data0/wmccorma/'+percval+'Sig_'+massval+'MeV_synthsig_multifold_not125.npz', allow_pickle=True)
...
det_args = {'input_dim': len(X_gen[0]), 'dense_sizes': model_layer_sizes, 'patience': 10, 'filepath': '/data0/wmccorma/'+percval+'_'+massval+'_synthsig_multifold_not125_Step1_{}', 'save_weights_only': False, 'modelcheck_opts': {'save_best_only': True, 'verbose': 1}}

mc_args = {'input_dim': len(X_gen[0]), 'dense_sizes': model_layer_sizes, 'patience': 10, 'filepath': '/data0/wmccorma/'+percval+'_'+massval+'_synthsig_multifold_not125_Step2_{}', 'save_weights_only': False, 'modelcheck_opts': {'save_best_only': True, 'verbose': 1}}
...
multifold_ws = of.omnifold(X_gen, Y_gen, X_det, Y_det, wdata, winit, (ef.archs.dnn.DNN, det_args), (ef.archs.dnn.DNN, mc_args), fitargs, val=0.2, it=itnum, trw_ind=-2, weights_filename='/data0/wmccorma/Official_synthsig_multifold_'+percval+'_perc_'+massval+'_mass_not125')
