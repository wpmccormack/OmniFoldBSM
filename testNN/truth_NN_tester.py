...
json_file = open('/data0/wmccorma/4000_SB_150_Jan_27.json', 'r')
loaded_model_json = json_file.read()
json_file.close()
loaded_model = model_from_json(loaded_model_json)
# load weights into new model
loaded_model.load_weights("/data0/wmccorma/4000_SB_150_Jan_27_labels.h5")
print("Loaded model from disk")

loaded_model.compile(loss='binary_crossentropy', optimizer='adam', metrics=['accuracy'])

fi = np.load('4000_SLIMTRUTH.npz', allow_pickle=True)
X_det = fi['X_det'][:]
Y_det = fi['Y_det'][:]

fi2 = np.load('/data0/wmccorma/10PercSig_3MeV_synthsig.npz', allow_pickle=True)
...
np.savez('4000MeV_10Perc_SB_150_Jan_27_preds_DATA.npz', **{'preds_half_true': preds, 'fpr_half_true': fpr, 'tpr_half_true': tpr, 'th_half_true': th, 'sif_half_true': sif, 'preds_data': preds2, 'fpr_data': fpr, 'tpr_data': tpr, 'th_data': th, 'sif_data': sif, 'preds_gen': preds3})
